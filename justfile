...
test:
    uv run pytest tests/unit tests/property tests/contract tests/component -n auto --tb=short

test-all:
    uv run pytest tests -n auto --tb=short

coverage:
    uv run pytest tests/unit tests/property tests/contract tests/component --cov=omniagent --cov-report=term-missing --cov-report=xml
...
help:
    ...
    @echo "  make test       - fast tests (unit, property, contract, component)"
