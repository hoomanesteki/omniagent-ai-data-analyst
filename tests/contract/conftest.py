"""Shared fixtures for contract tests: a small, hand-computed ecommerce warehouse.

Values are chosen so every metric's expected result can be computed by hand and
asserted exactly, rather than trusting the code under test to grade itself.
"""
...
def ecommerce_warehouse(tmp_path):
    ...
    Orders: O1/O2 (C1, completed, web), O3 (C2, cancelled — excluded from every
    metric filtered on order_status=completed), O4 (C3, completed, mobile_app).
    Completed gross_revenue = 100 + 50 + 75 = 225. order_count = 3.

    Returns: R1 (approved, on O1, refund 20), R2 (pending, on O2 — excluded).
    refunds = 20. net_revenue = 225 - 20 = 205. return_count = 1.
    return_rate = 1/3.
    ...
    conn.execute("""
        ...
        ('O1', 'C1', '2026-01-20', 'completed', 'web', 'card', 'US', NULL, 100.0),
        ('O2', 'C1', '2026-02-10', 'completed', 'web', 'card', 'US', NULL, 50.0),
    ...
    conn.execute("""
        ...
        ('R1', 'O1', 'defective', 'approved', '2026-01-25', 20.0),
        ('R2', 'O2', 'wrong_size', 'pending', '2026-02-15', 10.0)
    """)
    conn.close()
